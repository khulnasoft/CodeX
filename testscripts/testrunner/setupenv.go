...
func setupTestEnv(t *testing.T, envs *testscript.Env) error {
	...
	envs.Setenv(debug.CodexDebug, os.Getenv(debug.CodexDebug))
	return nil
}
...
func setupPATH(envs *testscript.Env) {
	// Ensure path is empty so that we rely only on the PATH set by codex
	// itself.
	...
	// us to call our own custom "codex" command.
...
func setupCacheHome(envs *testscript.Env) error {
	// Both codex itself and nix occasionally create some files in
	...
	err = os.MkdirAll(xdg.CacheSubpath("codex-tests/nix"), 0o755) // Ensure dir exists.
	if err != nil {
		return err
	}
	err = os.Symlink(xdg.CacheSubpath("codex-tests/nix"), filepath.Join(cacheHome, "nix"))
	if err != nil {
		return err
	}
