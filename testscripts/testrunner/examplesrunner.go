...
// xdgStateHomeDir is the home directory for codex state. We store symlinks to
// virtenvs of codex plugins in this directory. We need to use a custom
...
const xdgStateHomeDir = "/tmp/codex-testscripts"

// RunCodexTestscripts generates and runs a testscript test for each Codex project in dir.
// For each project, runs `codex run run_test` (if script exists) and asserts it succeeds.
func RunCodexTestscripts(t *testing.T, dir string) {
	// ensure the state home dir for codex exists
	...
	err = filepath.WalkDir(dir, func(path string, entry os.DirEntry, err error) error {
		...
		configPath := filepath.Join(path, "codex.json")
		config, err := devconfig.Open(configPath)
		if err != nil {
			// skip directories that do not have a codex.json defined
		...
		runSingleCodexTestscript(t, dir, path)
		return nil
	})
	if err != nil {
		t.Error(err)
...
func runSingleCodexTestscript(t *testing.T, dir, projectDir string) {
	...
	params.Setup = func(envs *testscript.Env) error {
		...
		// Reason: codex plugins like postgres store unix socket files in their state dir.
		envs.Setenv(envir.XDGStateHome, xdgStateHomeDir)

		// setup the codex testscript environment
		if err := setup(envs); err != nil {
			return errors.WithStack(err)
		}

		// copy all the files and folders of the codex-project being tested to the workdir
		...
		cmd := exec.Command("rm", "-rf", projectDir+"/.codex")
		err = cmd.Run()
		if err != nil {
...
// testscript file (.test.txt) for all codex-projects in the dir.
...
func generateTestscript(t *testing.T, dir, projectDir string) (string, error) {
	...
	// scriptName is the generic script file used for all codex projects
	...
	// for each Example codex-project.
