...
func assertCodexJSONPackagesContains(script *testscript.TestScript, neg bool, args []string) {
	if len(args) != 2 {
		script.Fatalf("usage: codexjson.packages.contains codex.json value")
	}
...
func assertCodexLockPackagesContains(script *testscript.TestScript, neg bool, args []string) {
	if len(args) != 2 {
		script.Fatalf("usage: codexlock.packages.contains codex.lock value")
	}
