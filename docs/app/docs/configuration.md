---
title: codex.json Reference
sidebar_position: 5
---

Your codex configuration is stored in a `codex.json` file, located in your project's root directory. This file can be edited directly, or using the [codex CLI](cli_reference/codex.md).
...
Note that `codex add` will automatically format `packages` based on the options and packages that you provide.

#### Pinning a Specific Version of a Package

You can pin a specific version of a package by adding a `@` followed by the version number to the end of the package name. For example, to pin the `go` package to version `1.19`, you can run `codex add go@1.19`, or add `go@1.19` to the packages list in your `codex.json`:
...
To see a list of packages and their available versions, you can run `codex search <pkg>`.

#### Adding Packages from Flakes

You can add packages from flakes by adding a reference to the  flake in the `packages` list in your `codex.json`. We currently support installing Flakes from Github and local paths.
...
This is a a map of key-value pairs that should be set as Environment Variables when activating `codex shell`, running a script with `codex run`, or starting a service. These variables will only be set in your Codex shell, and will have precedence over any environment variables set in your local machine or by [Codex Plugins](guides/plugins.md).

For example, you could set variable `$FOO` to `bar` by adding the following to your `codex.json`:
...
The Shell object defines init hooks and scripts that can be run with your shell. Right now two fields are supported: `init_hook`, which run a set of commands every time you start a codex shell, and `scripts`, which are commands that can be run using `codex run`
...
The init hook will run every time a new shell is started using `codex shell` or `codex run`, and is best used for setting up environment variables, aliases, or other quick setup steps needed to configure your environment. For longer running tasks, you should consider using a Script.

This is an example `codex.json` that customizes the prompt and prints a welcome message:
...
```json
{
    "shell": {
        "init_hook": [
            "export PS1='📦 codex> '",
            "echo 'Welcome! See CONTRIBUTING.md for tips on contributing to codex.'"
        ]
    }
}
```
...
Welcome! See CONTRIBUTING.md for tips on contributing to codex.
📦 codex>
```

#### Scripts

Scripts are commands that are executed in your Codex shell using `codex run <script_name>`. They can be used to start up background process (like databases or servers), or to run one off commands (like setting up a dev DB, or running your tests).
...
### Example: A Rust Codex

An example of a codex configuration for a Rust project called `hello_world` might look like the following:
