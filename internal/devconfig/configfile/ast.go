...
// configAST is a hujson syntax tree that represents a codex.json
// configuration. An AST allows the CLI to modify specific parts of a user's
// codex.json instead of overwriting the entire file. This is important
// because a codex.json can have user comments that must be preserved when
// saving changes.
//
...
//     the AST.
//
// Be aware that there are 4 ways of representing a package in codex.json that
...
// parseConfig parses the bytes of a codex.json and returns a syntax tree.
