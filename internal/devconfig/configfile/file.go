...
const (
	DefaultName = "codex.json"
)

// ConfigFile defines a codex environment as JSON.
type ConfigFile struct {
	// AbsRootPath is the absolute path to the codex.json or plugin.json file
	...
	// PackagesMutator is the slice of Nix packages that codex makes available in
	...
	// Shell configures the codex shell environment.
...
func validateScripts(cfg *ConfigFile) error {
	...
	for k := range scripts {
		if strings.TrimSpace(k) == "" {
			return errors.New("cannot have script with empty name in codex.json")
		}
		if whitespace.MatchString(k) {
			return errors.Errorf(
				"cannot have script name with whitespace in codex.json: %s", k)
		}
		if strings.TrimSpace(scripts[k].String()) == "" {
			return errors.Errorf(
				"cannot have an empty script body in codex.json: %s", k)
		}
	}
	return nil
}
