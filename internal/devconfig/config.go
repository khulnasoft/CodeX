...
import (
	...
	"github.com/khulnasoft/codex/internal/codex/shellcmd"
...
// ErrNotFound occurs when [Open] or [Find] cannot find a codex config file
// after searching a directory (and possibly its parent directories).
var ErrNotFound = errors.New("no codex config file found")
...
// Config represents a base codex.json as well as any included plugins it may have.
type Config struct {
...
const defaultInitHook = "echo 'Welcome to codex!' > /dev/null"

func DefaultConfig() *Config {
	cfg, err := loadBytes([]byte(fmt.Sprintf(`{
		"$schema": "https://raw.githubusercontent.com/khulnasoft/codex/%s/.schema/codex.schema.json",
		"packages": [],
		"shell": {
			"init_hook": [
				"%s"
	...
	if err != nil {
		panic("default codex.json is invalid: " + err.Error())
	}
	return cfg
}
...
// Open loads a Codex config from a file or project directory. If path is a
// directory, Open looks for a well-known config name (such as codex.json)
...
// name (such as codex.json), even if that config fails to load.
...
func searchDir(dir string) (*Config, error) {
	...
	for _, name := range try {
		...
		// Ignore directories named codex.json.
		if errors.Is(err, errIsDirectory) {
			continue
		}
...
func (c *Config) Packages(
	...
	// Packages to remove in built ins only affect the codex.json where they are defined.
