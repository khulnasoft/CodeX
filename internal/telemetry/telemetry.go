...
func Error(err error, meta Metadata) {
	...
	if sentryCtx := meta.envContext(); len(sentryCtx) > 0 {
		event.Contexts["Codex Environment"] = sentryCtx
	}
	...
	if sentryCtx := meta.pkgContext(); len(sentryCtx) > 0 {
		event.Contexts["Codex Packages"] = sentryCtx
	}
...
var (
	sentryBufferDir  = xdg.StateSubpath(filepath.FromSlash("codex/sentry"))
	segmentBufferDir = xdg.StateSubpath(filepath.FromSlash("codex/segment"))
)
...
func ShellStart() time.Time {
	return ParseShellStart(os.Getenv(envir.CodexShellStartTime))
}
