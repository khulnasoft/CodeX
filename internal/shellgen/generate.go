...
// codex.PrintEnv, which is the core function from which codex shell/run/direnv
// functionality is derived.
func GenerateForPrintEnv(ctx context.Context, codex codexer) error {
	defer trace.StartRegion(ctx, "GenerateForPrintEnv").End()

	plan, err := newFlakePlan(ctx, codex)
	if err != nil {
		return err
	}

	outPath := genPath(codex)
	...
	// Gitignore file is added to the .codex directory
	err = writeFromTemplate(filepath.Join(codex.ProjectDir(), ".codex"), plan, ".gitignore", ".gitignore")
	if err != nil {
		return errors.WithStack(err)
	}

	if plan.needsGlibcPatch() {
		patch, err := newGlibcPatchFlake(codex.Config().NixPkgsCommitHash(), plan.Packages)
		if err != nil {
			return redact.Errorf("generate glibc patch flake: %v", err)
		}
		if err := patch.writeTo(filepath.Join(FlakePath(codex), "glibc-patch")); err != nil {
			return redact.Errorf("write glibc patch flake to directory: %v", err)
		}
	}
	if err := makeFlakeFile(codex, plan); err != nil {
		return err
	}

	return WriteScriptsToFiles(codex)
}
...
func makeFlakeFile(d codexer, plan *flakePlan) error {
