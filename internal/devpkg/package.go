...
import (
	...
	"github.com/khulnasoft/codex/plugins"
)

// Package represents a "package" added to the codex.json config.
...
type Package struct {
	...
	// resolve resolves a Codex package string to a Nix installable.
	//
	...
	//
	// For flake packages (non-codex packages), resolve is a no-op.
	resolve func() error

	// Raw is the codex package name from the codex.json config.
	// Raw has a few forms:
	// 1. Codex Packages
...
func newPackage(raw string, isInstallable func() bool, locker lock.Locker) *Package {
	...
	// The raw string is either a Codex package ("name" or "name@version")
	// or it's a flake installable. In some cases they're ambiguous
	// ("nixpkgs" is a codex package and a flake). When that happens, we
	// assume a Codex package.
	parsed, err := flake.ParseInstallable(raw)
	if err != nil || pkgtype.IsAmbiguous(raw, parsed) {
		// TODO: This sets runx packages as codex packages. Not sure if that's what we want.
		pkg.IsCodexPackage = true
		pkg.resolve = sync.OnceValue(func() error { return resolve(pkg) })
		return pkg
	}

	// We currently don't lock flake references in codex.lock, so there's
	// nothing to resolve.
...
func (p *Package) NormalizedCodexPackageReference() (string, error) {
	if err := p.resolve(); err != nil {
		return "", err
	}
...
// During happy paths (codex packages and nix flakes that contains a fragment)
// it is much faster than NormalizedPackageAttributePath
func (p *Package) FullPackageAttributePath() (string, error) {
	if p.IsCodexPackage {
		reference, err := p.NormalizedCodexPackageReference()
		if err != nil {
			return "", err
		}
...
// it only applies to codex packages
...
func (p *Package) Versioned() string {
	if p.IsCodexPackage && !p.isVersioned() {
		return p.Raw + "@latest"
	}
	return p.Raw
}
...
func (p *Package) IsLegacy() bool {
	return p.IsCodexPackage && !p.isVersioned() && p.lockfile.Get(p.Raw).GetSource() == ""
}
...
func (p *Package) isVersioned() bool {
	return p.IsCodexPackage && strings.Contains(p.Raw, "@")
}
...
func (p *Package) EnsureUninstallableIsInLockfile() error {
	...
	// TODO savil: Do we need the IsCodexPackage check here?
	if !p.IsInstallable() || !p.IsCodexPackage {
		return nil
	}
...
// specified in codex.json package fields or as part of the flake reference.
...
func (p *Package) GetStorePaths(ctx context.Context, w io.Writer) ([]string, error) {
	...
	if featureflag.TidyWarning.Enabled() && p.IsCodexPackage {
...
// packageInstallErrorHandler checks for two kinds of errors to print custom messages for so that Codex users
...
func packageInstallErrorHandler(err error, pkg *Package, installableOrEmpty string) error {
	...
	// The user should try `codex add` again with `--exclude-platform`
	errMessage := strings.TrimSpace(err.Error())

	// Sample error from `codex add glibcLocales` on a mac:
	...
	// Sample error from `codex add sublime4` on a mac:
	...
	if maybePackageSystemCompatibilityErrorType1 || maybePackageSystemCompatibilityErrorType2 {
		...
		return usererr.WithUserMessage(
			...
			"package %s cannot be installed on your platform %s.\n"+
				...
				"you could run `codex add %[1]s --exclude-platform %[2]s` and re-try.\n"+
				...
				"You could try `codex add` with a different version for this package.\n\n"+
