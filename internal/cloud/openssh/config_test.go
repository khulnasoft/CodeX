...
func TestCodexIncludeRegex(t *testing.T) {
	tests := map[string]bool{
		`Include ~/.config/codex/ssh/config`:       true,
		`include ~/.config/codex/ssh/config`:       true,
		`Include "~/.config/codex/ssh/config"`:     true,
		`"Include" ~/.config/codex/ssh/config`:     true,
		`"Include" "~/.config/codex/ssh/config"`:   true,
		...
		`# Include ~/.config/codex/ssh/config`:                false,
		`Include ~/.config/blah # ~/.config/codex/ssh/config`: false,
		`Include`:                            false,
		`Include ~/.config/blah`:             false,
		`Include~/.config/codex/ssh/config`: false,
		`IdentityFile ~/.config/codex/ssh/config/keys/mykey`: false,
		`Hostname include codex/ssh/config`:                  false,
	}
	for in, match := range tests {
		t.Run(in, func(t *testing.T) {
			got := reCodexInclude.MatchString(in)
			if got != match {
...
//go:embed testdata/codex-ssh-config.golden
var goldenCodexSSHConfig []byte

func TestSetupCodex(t *testing.T) {
	want := fstest.MapFS{
		".config":            &fstest.MapFile{Mode: fs.ModeDir | 0o755},
		".config/codex":     &fstest.MapFile{Mode: fs.ModeDir | 0o755},
		".config/codex/ssh": &fstest.MapFile{Mode: fs.ModeDir | 0o700},
		".config/codex/ssh/config": &fstest.MapFile{
			Data: goldenCodexSSHConfig,
			Mode: 0o644,
		},
		".config/codex/ssh/known_hosts": &fstest.MapFile{
			Data: sshKnownHosts,
			Mode: 0o644,
		},
		".config/codex/ssh/sockets": &fstest.MapFile{Mode: fs.ModeDir | 0o700},

		".ssh": &fstest.MapFile{Mode: fs.ModeDir | 0o700},
		".ssh/config": &fstest.MapFile{
			Data: []byte("Include \"$HOME/.config/codex/ssh/config\"\n"),
			Mode: 0o644,
		},
	}
	...
	t.Run("NoConfigs", func(t *testing.T) {
		...
		if err := SetupCodex(); err != nil {
			t.Error("got SetupCodex() error:", err)
		}
	...
	t.Run("ExistingSSHConfig", func(t *testing.T) {
		...
		want[".ssh/config"] = &fstest.MapFile{
			Data: append(
				...
				[]byte("Include \"$HOME/.config/codex/ssh/config\"\n"),
				existingSSHConfig...,
			),
			Mode: 0o644,
		}
...
//go:embed testdata/codex-ssh-debug-config.golden
var goldenCodexSSHDebugConfig []byte
...
func TestSetupInsecureDebug(t *testing.T) {
	...
	want := fstest.MapFS{
		...
		".config/codex/ssh": &fstest.MapFile{Mode: fs.ModeDir | 0o700},
		".config/codex/ssh/config": &fstest.MapFile{
			Data: goldenCodexSSHDebugConfig,
			Mode: 0o644,
		},
		".config/codex/ssh/known_hosts": &fstest.MapFile{
			Data: sshKnownHosts,
		...
		".config/codex/ssh/known_hosts_debug": &fstest.MapFile{Mode: 0o644},
		".config/codex/ssh/sockets":           &fstest.MapFile{Mode: fs.ModeDir | 0o700},
	...
	t.Run("ChangeHost", func(t *testing.T) {
		...
		input[".config/codex/ssh/config"] = &fstest.MapFile{
			Data: bytes.ReplaceAll(goldenCodexSSHDebugConfig, []byte("127.0.0.1"), []byte("127.0.0.2")),
			Mode: 0o644,
		}

		// Put something in known_hosts_debug so we can check that it
		// gets cleared out.
		input[".config/codex/ssh/known_hosts_debug"] = &fstest.MapFile{
...
//go:embed testdata/test.vm.codex-vms.internal.golden
var goldenVMKey []byte

func TestAddVMKey(t *testing.T) {
	host := "test.vm.codex-vms.internal"
	input := fstest.MapFS{}
	want := fstest.MapFS{
		".config":                 &fstest.MapFile{Mode: fs.ModeDir | 0o755},
		".config/codex":          &fstest.MapFile{Mode: fs.ModeDir | 0o755},
		".config/codex/ssh":      &fstest.MapFile{Mode: fs.ModeDir | 0o700},
		".config/codex/ssh/keys": &fstest.MapFile{Mode: fs.ModeDir | 0o700},

		".config/codex/ssh/keys/" + host: &fstest.MapFile{
			Data: goldenVMKey,
			Mode: 0o600,
		},
	}
