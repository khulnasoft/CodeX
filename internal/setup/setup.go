...
type RunInfo struct {
	...
	// Version is the version of Codex that last ran the task.
...
const (
	...
	// process due to the task calling [SudoCodex] from the parent process.
	TaskSudoing
)
...
// SudoCodex relaunches Codex as root using sudo, taking care to preserve
// Codex environment variables that can affect the new process. If the current
...
//		ran, err := SudoCodex(ctx, "cache", "configure")
//		if ran || err != nil {
...
//	}
//
//	ConfirmRun(ctx, key, &ConfigTask{}, "Allow sudo to run Codex as root?")
//
// A task that calls SudoCodex should pass command arguments that cause the new
// Codex process to rerun the task. The task executes unconditionally within
// the sudo process without re-prompting the user or a second call to its
// NeedsRun method.
func SudoCodex(ctx context.Context, arg ...string) (ran bool, err error) {
	if os.Getuid() == 0 {
		return false, nil
	}
	...
	// Use the absolute path to Codex instead of relying on PATH for two
	// reasons:
	//
	//  1. sudo isn't guaranteed to preserve the current PATH and the root
	//     user might not have codex in its PATH.
	//  2. If we're running an alternative version of Codex
	...
	sudoArgs = append(sudoArgs, "--preserve-env="+strings.Join([]string{
		...
		// Use the same Codex API and auth token.
		"CODEX_API_TOKEN",
		"CODEX_PROD",

		// In case the Codex version is overridden.
		"CODEX_USE_VERSION",
	...
	if taskKey != "" {
		sudoArgs = append(sudoArgs, "CODEX_SUDO_TASK="+taskKey)
	}
...
func isSudo(key string) bool {
	// CODEX_SUDO_TASK is set when a task relaunched Codex by calling
	// SudoCodex. If it matches the current task key, then the pre-sudo
	// process is already running this task and we can skip checking
	// task.NeedsRun and prompting the user.
	envTask := os.Getenv("CODEX_SUDO_TASK")
