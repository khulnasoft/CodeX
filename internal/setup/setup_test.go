...
// TestSudoCodex uses sudo on the current test binary to recursively call
...
//   - The sudo version of the test creates a "test-sudo-codex-result" file.
//   - The non-sudo version of the test looks for the same file to know if the
//     sudo worked.
func TestSudoCodex(t *testing.T) {
	...
	task.RunFunc = func(ctx context.Context) error {
		ran, err := SudoCodex(ctx, "-test.run", "^"+t.Name()+"$")
		if ran || err != nil {
			return err
		}
		...
		err = fmt.Errorf("task.NeedsRun not running as root after calling SudoCodex")
		t.Error(err)
		return err
	}
	task.NeedsRunFunc = func(ctx context.Context, lastRun RunInfo) bool {
	...
	err := ConfirmRun(ctx, key, task, "Allow sudo to run Codex as root?")
	if err != nil {
