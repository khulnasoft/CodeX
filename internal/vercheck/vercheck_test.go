...
func TestCheckVersion(t *testing.T) {
	...
	t.Run("skip_if_codex_cloud", func(t *testing.T) {
		defer os.Unsetenv(envName)
		// if codex cloud
		t.Setenv(envir.CodexRegion, "true")
		buf := new(bytes.Buffer)
		CheckVersion(buf, "codex shell")
		if buf.String() != "" {
			t.Errorf("expected empty string, got %q", buf.String())
		}
		t.Setenv(envir.CodexRegion, "")
	})
	...
	t.Run("skip_if_no_launcher_version_or_latest_version", func(t *testing.T) {
		...
		t.Setenv(envir.CodexLatestVersion, "")
		buf := new(bytes.Buffer)
		CheckVersion(buf, "codex shell")
		if buf.String() != "" {
			t.Errorf("expected empty string, got %q", buf.String())
	...
	t.Run("print_if_binary_version_outdated", func(t *testing.T) {
		...
		t.Setenv(envir.CodexLatestVersion, "0.4.9")

		// mock the existing binary version
		currentCodexVersion = "v0.4.8"

		buf := new(bytes.Buffer)
		CheckVersion(buf, "codex shell")
		if !strings.Contains(buf.String(), "New codex available") {
			t.Errorf("expected notice about new codex version, got %q", buf.String())
		}
	})
	...
	t.Run("skip_if_all_versions_up_to_date", func(t *testing.T) {
		...
		t.Setenv(envir.CodexLatestVersion, "0.4.8")
