...
func TestTemplatesExist(t *testing.T) {
	...
	for _, path := range templates {
		_, err := os.Stat(filepath.Join(curDir, path, "codex.json"))
		if errors.Is(err, os.ErrNotExist) {
			t.Errorf("Directory/codex.json for %s does not exist", path)
		}
	}
}

func TestParseRepoURL(t *testing.T) {
	// codex create --repo="http:::/not.valid/a//a??a?b=&&c#hi"
	...
	assert.Error(t, err)
	_, err = ParseRepoURL("git@github.com:khulnasoft/codex.git")
	assert.Error(t, err)
