...
func InitFromName(w io.Writer, template, target string) error {
	...
	return InitFromRepo(w, "https://github.com/khulnasoft/codex", templatePath, target)
}
...
func InitFromRepo(w io.Writer, repo, subdir, target string) error {
	...
	tmp, err := os.MkdirTemp("", "codex-template")
	if err != nil {
		return errors.WithStack(err)
	}
	cmd := exec.Command(
...
func ParseRepoURL(repo string) (string, error) {
	...
	// like: https://github.com/khulnasoft/codex.git
