...
// not changed. This can happen when doing `codex update` and search has
	...
	for sys, info := range resolved.Systems {
		if len(info.Outputs) != 0 {
			...
			if len(outputs) > 0 {
				...
				// Codex used to do. And this code is for backwards-compatibility.
				//
				...
				// install both outputs. So, team members on older Codex will see just `bin` installed while
				// team members on newer Codex will see both `bin` and `man` installed.
