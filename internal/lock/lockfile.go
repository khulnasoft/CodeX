...
import (
	...
	"github.com/khulnasoft/codex/lib/pkg/runx/impl/types"
...
func GetFile(project codexProject) (*File, error) {
	lockFile := &File{
...
func (f *File) Resolve(pkg string) (*Package, error) {
	...
	if !hasEntry || entry.Resolved == "" {
		...
		if _, _, versioned := searcher.ParseVersionedPackage(pkg); pkgtype.IsRunX(pkg) || versioned {
			...
			// whatever hash is in the codex.json
			locked = &Package{
...
func (f *File) Save() error {
	...
	// Reason: We want to update `codex.lock` file only upon a user action
	// such as `codex update` or `codex add` or `codex remove`.
	...
	return cuecfg.WriteFile(lockFilePath(f.codexProject.ProjectDir()), f)
}
...
// Tidy ensures that the lockfile has the set of packages corresponding to the codex.json config.
// It gets rid of older packages that are no longer needed.
func (f *File) Tidy() {
	f.Packages = lo.PickByKeys(
		f.Packages,
		f.codexProject.AllPackageNamesIncludingRemovedTriggerPackages(),
	)
}
...
func (f *File) IsUpToDateAndInstalled(isFish bool) (bool, error) {
	...
	configHash, err := f.codexProject.ConfigHash()
	if err != nil {
		return false, err
	}
	return isStateUpToDate(UpdateStateHashFileArgs{
		ProjectDir: f.codexProject.ProjectDir(),
...
func (f *File) isDirty() (bool, error) {
	...
	fileSystemLockFile, err := GetFile(f.codexProject)
	if err != nil {
		return false, err
	}
...
func lockFilePath(projectDir string) string {
	return filepath.Join(projectDir, "codex.lock")
}
