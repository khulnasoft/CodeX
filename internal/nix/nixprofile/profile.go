...
func ProfileListNameOrIndex(args *ProfileListNameOrIndexArgs) (string, error) {
	...
	if !inCache && args.Package.IsCodexPackage {
		// This is an optimization for happy path when packages are added by flake reference. A resolved codex
		...
		ref, err := args.Package.NormalizedCodexPackageReference()
		if err != nil {
