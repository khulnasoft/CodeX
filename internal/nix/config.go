...
func IncludeCodexConfig(ctx context.Context, username string) error {
	info, _ := versionInfo()
	path := cmp.Or(info.SystemConfig, "/etc/nix/nix.conf")
	includePath := filepath.Join(filepath.Dir(path), "codex-nix.conf")
	b := fmt.Appendf(nil, "# This config was auto-generated by Codex.\n\nextra-trusted-users = %s\n", username)
	if err := os.WriteFile(includePath, b, 0o664); err != nil {
		return redact.Errorf("write codex nix.conf: %v", err)
	}
