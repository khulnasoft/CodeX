...
func EnsureNixInstalled(writer io.Writer, withDaemonFunc func() *bool) (err error) {
	...
	defer func() {
		...
		if !version.AtLeast(MinVersion) {
			err = usererr.New(
				"Codex requires nix of version >= %s. Your version is %s. "+
	...
	if dirExists() {
		...
		return usererr.New(
			"We found a /nix directory but nix binary is not in your PATH and we " +
				...
				"https://github.com/khulnasoft/codex/issues",
		)
	}

	color.Yellow("\nNix is not installed. Codex will attempt to install it.\n\n")
	...
	fmt.Fprintln(writer, "Nix installed successfully. Codex is ready to use!")
	return nil
}
