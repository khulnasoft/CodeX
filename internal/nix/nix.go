...
// Instead of using directory, prefer using the codex.ProfileDir() function that ensures the directory exists.
const ProfilePath = ".codex/nix/profile/default"
...
func ComputeSystem() error {
	...
	// So, I need to fake that I am x86-linux and inspect the output in generated codex.lock
	// and flake.nix files.
	// This is also used by unit tests.
	if cachedSystem != "" {
		return nil
	}
	override := os.Getenv("__CODEX_NIX_SYSTEM")
	if override != "" {
...
// All major Nix versions supported by Codex.
const (
	Version2_12 = "2.12.0"
	Version2_13 = "2.13.0"
	Version2_14 = "2.14.0"
...
var nixPlatforms = []string{
	...
	// ref. https://github.com/khulnasoft/codex/pull/1300
	"armv7l-linux",
}
...
// produced by the flakes.nix. Use codex.NixBins() instead.
...
func IsExitErrorInsecurePackage(err error, pkgNameOrEmpty, installableOrEmpty string) (bool, error) {
	...
	if errors.As(err, &exitErr) && exitErr.ExitCode() == 1 {
		if strings.Contains(string(exitErr.Stderr), "is marked as insecure") {
			...
			errMessages = append(errMessages,
				fmt.Sprintf("To override, use `codex add %s --allow-insecure=%s`", pkgName, strings.Join(insecurePackages, ", ")))
