...
const (
	// TODO rename to codexPluginUserConfigDirName
	codexDirName       = "codex.d"
	codexHiddenDirName = ".codex"
	pluginConfigName    = "plugin.json"
)

var (
	VirtenvPath    = filepath.Join(codexHiddenDirName, "virtenv")
...
type PluginOnlyData struct {
	...
	// 2. Plugins can be added via the "include" field in codex.json or plugin.json
	Source Includable
}
...
func (m *Manager) createFile(
	...
	if err = tmpl.Execute(&buf, map[string]any{
		"CodexDir":            filepath.Join(m.ProjectDir(), codexDirName, name),
		"CodexDirRoot":        filepath.Join(m.ProjectDir(), codexDirName),
		"CodexProfileDefault": filepath.Join(m.ProjectDir(), nix.ProfilePath),
...
func buildConfig(pkg Includable, projectDir, content string) (*Config, error) {
	...
	if err = t.Execute(&buf, map[string]string{
		"CodexProjectDir":     projectDir,
		"CodexDir":            filepath.Join(projectDir, codexDirName, name),
		"CodexDirRoot":        filepath.Join(projectDir, codexDirName),
		"CodexProfileDefault": filepath.Join(projectDir, nix.ProfilePath),
...
func (m *Manager) shouldCreateFile(
	...
	// Only create files in codex.d directory if they are not in the lockfile
	pluginInstalled := pkg != nil && pkg.PluginVersion != ""
	if strings.Contains(filePath, sep+codexDirName+sep) && pluginInstalled {
		return false
	}

	// Hidden .codex files are always replaceable, so ok to recreate
	if strings.Contains(filePath, sep+codexHiddenDirName+sep) {
		return true
	}
