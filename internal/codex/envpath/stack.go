...
const (
	...
	// codexEnvPath for that codex-project. Except for the last element, which is InitPathEnv.
	PathStackEnv = "CODEX_PATH_STACK"

	// InitPathEnv stores the path prior to any codex shellenv modifying the environment
	InitPathEnv = "CODEX_INIT_PATH"
)

// stack has the following design:
// 1. The stack enables tracking which sub-paths in PATH come from which codex-project
// 2. It is an ordered-list of keys to env-vars that store codexEnvPath values of codex-projects.
// 3. The final PATH is reconstructed by concatenating the env-var values of each of these keys.
// 4. The stack is stored in its own env-var PathStackEnv, shared by all codex-projects in this shell.
type stack struct {
...
func Stack(env, originalEnv map[string]string) *stack {
	...
	if !ok || strings.TrimSpace(stackEnv) == "" {
		...
		// external environment prior to any codex-shellenv being applied to it.
...
// Key is the element stored in the stack for a codex-project. It represents
// a pointer to the codexEnvPath value stored in its own env-var, also using this same Key.
func Key(projectHash string) string {
	return "CODEX_NIX_ENV_PATH_" + projectHash
}

// Push adds the new PATH for the codex-project identified by projectHash.
...
func (s *stack) Push(
	...
	projectHash string,
	path string, // new PATH of the codex-project of projectHash
