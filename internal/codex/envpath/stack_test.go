	...
	for idx, testStep := range testSteps {
		t.Run(
			fmt.Sprintf("step_%d", idx), func(t *testing.T) {
				...
				stack.Push(env, testStep.projectHash, testStep.codexEnvPath, testStep.preservePathStack)
