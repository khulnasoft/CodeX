...
// CodexShell configures a user's shell to run in Codex. Its zero value is a
// fallback shell that launches a regular Nix shell.
type CodexShell struct {
	codex          *Codex
	name            name
	binPath         string
	projectDir      string // path to where codex.json config resides
...
type ShellOption func(*CodexShell)

// NewCodexShell initializes the CodexShell struct so it can be used to start a shell environment
// for the codex project.
func NewCodexShell(codex *Codex, envOpts devopt.EnvOptions, opts ...ShellOption) (*CodexShell, error) {
	shPath, err := shellPath(codex, envOpts)
	if err != nil {
		return nil, err
	}
...
func shellPath(codex *Codex, envOpts devopt.EnvOptions) (path string, err error) {
	defer func() {
		if err != nil {
	...
	cmd := exec.Command(
		"nix", "eval", "--raw",
		fmt.Sprintf("%s#bashInteractive", nix.FlakeNixpkgs(codex.cfg.NixPkgsCommitHash())),
	)
...
// for the codex shell.
func initShellBinaryFields(path string) *CodexShell {
	shell := &CodexShell{binPath: path}
...
func (s *CodexShell) Run() error {
	var cmd *exec.Cmd
	shellrc, err := s.writeCodexShellrc()
	if err != nil {
		...
		slog.Error("failed to write codex shellrc", "err", err)
		return errors.WithStack(err)
	}
	...
	// This means that there was an error from codex's code or nix's code. Not a user
	// error and so we do return it.
	return errors.WithStack(err)
}

func (s *CodexShell) shellRCOverrides(shellrc string) (extraEnv map[string]string, extraArgs []string) {
...
func (s *CodexShell) writeCodexShellrc() (path string, err error) {
	...
	if s.userShellrcPath != "" {
		// If we can't read it, then just omit it from the codex shellrc.
		userShellrc, _ = os.ReadFile(s.userShellrcPath)
	}

	// If the user already has a shellrc file, then give the codex shellrc
	...
	err = tmpl.Execute(shellrcf, struct {
		...
		RefreshAliasName:   s.codex.refreshAliasName(),
		RefreshCmd:         s.codex.refreshCmd(),
		RefreshAliasEnvVar: s.codex.refreshAliasEnvVar(),
	})
	if err != nil {
		return "", fmt.Errorf("execute shellrc template: %v", err)
	}

	slog.Debug("wrote codex shellrc", "path", path)
	return path, nil
}
...
// within the codex shell.
//
// We do not link the .{shell}rc files, since codex modifies them. See writeCodexShellrc
func (s *CodexShell) linkShellStartupFiles(shellSettingsDir string) {
