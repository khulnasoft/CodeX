...
import (
	...
	"github.com/khulnasoft/codex/internal/nix/nixprofile"
...
func (d *Codex) Update(ctx context.Context, opts devopt.UpdateOpts) error {
	...
	for _, pkg := range pendingPackagesToUpdate {
		if _, _, isVersioned := searcher.ParseVersionedPackage(pkg.Raw); !isVersioned {
			...
			if err = d.updateCodexPackage(pkg); err != nil {
				return err
			}
		}
	}
	...
	// It will return an error if .codex/gen/flake is missing
...
func (d *Codex) updateCodexPackage(pkg *devpkg.Package) error {
...
func (d *Codex) mergeResolvedPackageToLockfile(
	pkg *devpkg.Package,
...
func (d *Codex) attemptToUpgradeFlake(pkg *devpkg.Package) error {
