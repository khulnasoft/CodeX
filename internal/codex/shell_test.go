...
func TestWriteCodexShellrc(t *testing.T) {
	testdirs, err := filepath.Glob("testdata/shellrc/*")
	if err != nil {
		t.Fatal("Error globbing testdata:", err)
	}
	testWriteCodexShellrc(t, testdirs)
}

func testWriteCodexShellrc(t *testing.T, testdirs []string) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			s := &CodexShell{
				codex:          &Codex{projectDir: projectDir},
				env:             test.env,
				projectDir:      "/path/to/projectDir",
				userShellrcPath: test.shellrcPath,
			}
			gotPath, err := s.writeCodexShellrc()
			if err != nil {
				t.Fatal("Got writeCodexShellrc error:", err)
			}
