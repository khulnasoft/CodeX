...
const StateOutOfDateMessage = "Your codex environment may be out of date. Run %s to update it.\n"

// packages.go has functions for adding, removing and getting info about nix
// packages

// Add adds the `pkgs` to the config (i.e. codex.json) and nix profile for this
// codex project
func (d *Codex) Add(ctx context.Context, pkgsNames []string, opts devopt.AddOpts) error {
	ctx, task := trace.NewTask(ctx, "codexAdd")
	defer task.End()
	...
	for _, pkg := range pkgs {
		...
		if slices.Contains(existingPackageNames, pkg.Versioned()) {
			...
			ux.Finfo(d.stderr, "Package %q already in codex.json\n", pkg.Versioned())
			continue
		}
		...
		if found != nil {
			ux.Finfo(d.stderr, "Replacing package %q in codex.json\n", found.Raw)
		...
		} else if !versionedPkg.IsCodexPackage {
...
func (d *Codex) setPackageOptions(pkgs []string, opts devopt.AddOpts) error {
	for _, pkg := range pkgs {
...
func (d *Codex) printPostAddMessage(
	ctx context.Context,
	pkgs []*devpkg.Package,
	unchangedPackageNames []string,
	opts devopt.AddOpts,
	...
	if len(opts.Platforms) == 0 && len(opts.ExcludePlatforms) == 0 && len(opts.Outputs) == 0 && len(opts.AllowInsecure) == 0 {
		if len(unchangedPackageNames) == 1 {
			ux.Finfo(d.stderr, "Package %q was already in codex.json and was not modified\n", unchangedPackageNames[0])
		} else if len(unchangedPackageNames) > 1 {
			ux.Finfo(d.stderr, "Packages %s were already in codex.json and were not modified\n",
...
// Remove removes the `pkgs` from the config (i.e. codex.json) and nix profile
// for this codex project
func (d *Codex) Remove(ctx context.Context, pkgs ...string) error {
	ctx, task := trace.NewTask(ctx, "codexRemove")
	defer task.End()
	...
	if len(missingPkgs) > 0 {
		ux.Fwarning(
			d.stderr,
			"the following packages were not found in your codex.json: %s\n",
...
// ensureStateIsUpToDate ensures the Codex project state is up to date.
// Namely:
...
//  3. Files for codex shellenv are generated
//  4. The Codex environment is re-computed, if necessary, and cached
...
func (d *Codex) ensureStateIsUpToDate(ctx context.Context, mode installMode) error {
	defer trace.StartRegion(ctx, "codexEnsureStateIsUpToDate").End()
	...
	// If we're in a codex shell (global or project), then the environment might
...
// updateLockfile will ensure codex.lock is up to date with the current state of the project.update
// If recomputeState is true, then we will also update the state.json file.
func (d *Codex) updateLockfile(recomputeState bool) error {
...
// - the generated flake
// - the nix-profile
func (d *Codex) recomputeState(ctx context.Context) error {
	defer debug.FunctionTimer().End()
...
func (d *Codex) profilePath() (string, error) {
...
// state if the Flakes feature has been changed, from the previous execution of a codex command.
...
func (d *Codex) installPackages(ctx context.Context, mode installMode) error {
	defer debug.FunctionTimer().End()
...
func (d *Codex) handleInstallFailure(ctx context.Context, mode installMode) error {
...
func (d *Codex) InstallRunXPackages(ctx context.Context) error {
...
// mode is install or update, and we're not in a codex environment.
// This is done by running `nix build` on the flake. We do this so that the
// packages will be available in the nix store when computing the codex environment
// and installing in the nix profile (even if offline).
func (d *Codex) installNixPackagesToStore(ctx context.Context, mode installMode) error {
	defer debug.FunctionTimer().End()
...
func (d *Codex) appendExtraSubstituters(ctx context.Context, args *nix.BuildArgs) error {
	creds, err := nixcache.CachedCredentials(ctx)
	if errors.Is(err, auth.ErrNotLoggedIn) {
		return nil
	}
	if err != nil {
		ux.Fwarning(d.stderr, "Codex was unable to authenticate with the Khulnasoft Nix cache. Some packages might be built from source.\n")
		return nil //nolint:nilerr
	}

	caches, err := nixcache.CachedReadCaches(ctx)
	if err != nil {
		slog.Error("error getting list of caches from the Khulnasoft API, assuming the user doesn't have access to any", "err", err)
		return nil
	}
	if len(caches) == 0 {
		return nil
	...
	if errors.Is(err, setup.ErrUserRefused) {
		ux.Finfo(d.stderr, "Skipping cache setup. Run `codex cache configure` to enable the cache at a later time.\n")
		return nil
	}
	var daemonErr *nix.DaemonError
	if errors.As(err, &daemonErr) {
		// Error here to give the user a chance to restart the daemon.
		return usererr.New("Codex configured Nix to use a new cache. Please restart the Nix daemon and re-run Codex.")
	}
	...
	if err != nil {
		...
		ux.Fwarning(d.stderr, "Codex was unable to configure Nix to use the Khulnasoft Nix cache. Some packages might be built from source.\n")
		return nil
	}
...
func (d *Codex) packagesToInstallInStore(ctx context.Context, mode installMode) ([]*devpkg.Package, error) {
	defer debug.FunctionTimer().End()
...
// moveAllowInsecureFromLockfile will modernize a Codex project by moving the allow_insecure: boolean
// setting from the codex.lock file to the corresponding package in codex.json.
//
// NOTE: ideally, this function would be in devconfig, but it leads to an import cycle with devpkg, so
// leaving in this "top-level" codex package where we can import devconfig, devpkg and lock.
func (d *Codex) moveAllowInsecureFromLockfile(writer io.Writer, lockfile *lock.File, cfg *devconfig.Config) error {
	...
	// Set the codex.json packages to allow_insecure
	for _, versionedName := range insecurePackages {
		...
		if err := cfg.PackageMutator().SetAllowInsecure(writer, versionedName, []string{storeName}); err != nil {
			return fmt.Errorf("failed to set allow_insecure in codex.json for package %q with error %w", versionedName, err)
		}
	}
	...
	ux.Finfo(
		writer,
		"Modernized the allow_insecure setting for package %q by moving it from codex.lock to codex.json. Please commit the changes.\n",
...
func (d *Codex) FixMissingStorePaths(ctx context.Context) error {
	packages := d.InstallablePackages()
	for _, pkg := range packages {
		if !pkg.IsCodexPackage || pkg.IsRunX() {
			continue
		}
