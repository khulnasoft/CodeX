...
// package generate has functionality to implement the `codex generate` command
...
func (g *Options) CreateDockerfile(
	...
	return t.Execute(file, map[string]any{
		...
		"CodexRunInstall": lo.Ternary(opts.HasInstall, "codex run install", "echo 'No install script found, skipping'"),
		"CodexRunBuild":   lo.Ternary(opts.HasBuild, "codex run build", "echo 'No build script found, skipping'"),
		"Cmd":              fmt.Sprintf("%q, %q, %q", "codex", "run", "start"),
	})
}
...
func (g *Options) getDevcontainerContent() *devcontainerObject {
	...
	for _, pkg := range g.Pkgs {
		if py3pattern.MatchString(pkg) {
			// Setup python3 interpreter path to codex in the container
			devcontainerContent.Customizations.Vscode.Settings = map[string]any{
				"python.defaultInterpreterPath": "/code/.codex/nix/profile/default/bin/python3",
			}
