...
func (d *Codex) StartServices(
	...
	if !services.ProcessManagerIsRunning(d.projectDir) {
		...
		fmt.Fprintln(d.stderr, "\nNOTE: We recommend using `codex services up` to start process-compose and your services")
...
func (d *Codex) StopServices(ctx context.Context, runInCurrentShell, allProjects bool, serviceNames ...string) error {
	if !runInCurrentShell {
		...
		return d.runCodexServicesScript(ctx, args)
	}
	...
	if !services.ProcessManagerIsRunning(d.projectDir) {
		return usererr.New("Process manager is not running. Run `codex services up` to start it.")
	}
...
func (d *Codex) ListServices(ctx context.Context, runInCurrentShell bool) error {
	if !runInCurrentShell {
		return d.runCodexServicesScript(ctx, []string{"ls", "--run-in-current-shell"})
	}
	...
	if !services.ProcessManagerIsRunning(d.projectDir) {
		fmt.Fprintln(d.stderr, "No services currently running. Run `codex services up` to start them:")
		fmt.Fprintln(d.stderr, "")
...
func (d *Codex) RestartServices(
	...
	if !services.ProcessManagerIsRunning(d.projectDir) {
		fmt.Fprintln(d.stderr, "Process-compose is not running. Starting it now...")
		fmt.Fprintln(d.stderr, "\nTip: We recommend using `codex services up` to start process-compose and your services")
		return d.StartProcessManager(ctx, runInCurrentShell, serviceNames, devopt.ProcessComposeOpts{Background: true})
	}
...
func (d *Codex) StartProcessManager(
	ctx context.Context,
	...
	if !runInCurrentShell {
		...
		// TODO: Here we're attempting to reconstruct arguments from the original command, so that we can reinvoke it in codex shell.
	...
	err = initCodexUtilityProject(ctx, d.stderr)
	if err != nil {
		return err
	}
...
// runCodexServicesScript invokes RunScript with the envOptions set to the appropriate
// defaults for the `codex services` scenario.
func (d *Codex) runCodexServicesScript(ctx context.Context, cmdArgs []string) error {
	cmdArgs = append([]string{"services"}, cmdArgs...)
	return d.RunScript(ctx, devopt.EnvOptions{}, "codex", cmdArgs)
}
