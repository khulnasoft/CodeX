...
import (
	...
	nixv1alpha1 "github.com/khulnasoft/codex/lib/pkg/api/gen/priv/nix/v1alpha1"
)
...
func cacheCmd() *cobra.Command {
	...
	uploadCommand := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if len(args) > 0 {
				return codex.UploadInstallableToCache(
...
func cacheConfigureCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "configure",
		Short: "Configure Nix to use the Codex cache as a substituter",
		Long: heredoc.Doc(`
...
func cacheEnableCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "enable",
		Short: "Enable the Codex Nix cache for your account",
		Long: heredoc.Doc(`
			Sign up or log in to a Khulnasoft Cloud account and configure Nix to use the
			account's Nix cache.

			For more about how Codex configures Nix, see "codex cache configure -h".
		`),
		Args: cobra.ExactArgs(0),
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !needConfigure && !needLogin {
				fmt.Fprintln(cmd.OutOrStdout(), "The Codex cache is already enabled for your account.")
			}
			return nil
		},
	}
