...
import (
	...
	"github.com/khulnasoft/codex/internal/boxcli/midcobra"
	...
	"github.com/khulnasoft/codex/internal/vercheck"
)
...
func Main() {
	...
	if len(os.Args) > 1 && os.Args[1] == "upload-telemetry" {
		// This subcommand is hidden and only run by codex itself as a
...
func setSystemBinaryPaths() {
	if os.Getenv("CODEX_SYSTEM_BASH") == "" {
		os.Setenv("CODEX_SYSTEM_BASH", cmdutil.GetPathOrDefault("bash", "/bin/bash"))
	}
	if os.Getenv("CODEX_SYSTEM_SED") == "" {
		os.Setenv("CODEX_SYSTEM_SED", cmdutil.GetPathOrDefault("sed", "/usr/bin/sed"))
	}
}
