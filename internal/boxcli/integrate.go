...
func integrateVSCodeCmd() *cobra.Command {
	...
	command := &cobra.Command{
		Use:    "vscode",
		Hidden: true,
		Short:  "Integrate codex environment with VSCode.",
		RunE: func(cmd *cobra.Command, args []string) error {
...
func runIntegrateVSCodeCmd(cmd *cobra.Command, flags integrateCmdFlags) error {
	...
	dbug.logToFile("Codex process initiated. Setting up communication channel with VSCode process")
	...
	// Get env variables of a codex shell
	dbug.logToFile("Computing codex environment")
	...
	envVars = slices.DeleteFunc(envVars, func(s string) bool {
		...
		// CODEX_OG_PATH_<hash> being set causes codex global shellenv to overwrite the
		// PATH after VSCode opens and resets it to global shellenv. This causes the VSCode
		// terminal to not be able to find codex packages after the reopen in codex
		// environment action is called.
		return ok && (strings.HasPrefix(k, "CODEX_OG_PATH") || k == "HOME" || k == "NODE_CHANNEL_FD")
	})
	...
	// Open vscode with codex shell environment
	cmnd := exec.Command("code", message.ConfigDir)
	cmnd.Env = append(cmnd.Env, envVars...)
	var outb, errb bytes.Buffer
	cmnd.Stdout = &outb
	cmnd.Stderr = &errb
	dbug.logToFile("Re-opening VSCode in computed codex environment")
	err = cmnd.Run()
	if err != nil {
...
func (d *debugMode) logToFile(msg string) {
	...
	if d.enabled {
		file, err := os.OpenFile(".codex/extension.log", os.O_APPEND|os.O_WRONLY, 0o666)
		if err != nil {
			log.Fatal(err)
		}
