...
func runCmd(defaults runFlagDefaults) *cobra.Command {
	...
	command := &cobra.Command{
		...
		Long: "Start a new shell and runs your script or command in it, exiting when done.\n\n" +
			"The script must be defined in `codex.json`, or else it will be interpreted as an " +
	...
	command.Flags().BoolVar(
		&flags.pure, "pure", false, "if this flag is specified, codex runs the script in an isolated environment inheriting almost no variables from the current environment. A few variables, in particular HOME, USER and DISPLAY, are retained.")
	command.Flags().BoolVarP(
		&flags.listScripts, "list", "l", false, "list all scripts defined in codex.json")
	command.Flags().BoolVar(
...
func listScripts(cmd *cobra.Command, flags runCmdFlags) []string {
	...
	if err != nil {
		slog.Error("failed to open codex", "err", err)
		return nil
	}
...
func runScriptCmd(cmd *cobra.Command, args []string, flags runCmdFlags) error {
	if len(args) == 0 || flags.listScripts {
		...
		if len(scripts) == 0 {
			fmt.Fprintln(cmd.OutOrStdout(), "no scripts defined in codex.json")
			return nil
		}
	...
	if err != nil {
		return redact.Errorf("error reading codex.json: %w", err)
	}
	...
	if err := box.RunScript(cmd.Context(), envOpts, script, scriptArgs); err != nil {
		return redact.Errorf("error running script %q in Codex: %w", script, err)
	}
	return nil
}
