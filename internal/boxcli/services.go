...
func (flags *serviceUpFlags) register(cmd *cobra.Command) {
	cmd.Flags().StringVar(
		...
		"path to process compose file or directory containing process "+
			"compose-file.yaml|yml. Default is directory containing codex.json",
	)
	cmd.Flags().BoolVarP(
...
func servicesCmd(persistentPreRunE ...cobraFunc) *cobra.Command {
	...
	servicesCommand := &cobra.Command{
		Use:   "services",
		Short: "Interact with codex services.",
		Long: "Interact with codex services. Services start in a new shell. " +
			...
			"the --env or --env-file flag. You may also override in codex.json by " +
