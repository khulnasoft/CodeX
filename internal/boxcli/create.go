...
import (
	...
	"github.com/khulnasoft/codex/internal/templates"
...
func createCmd() *cobra.Command {
	...
	command := &cobra.Command{
		...
		Short: "Initialize a directory as a codex project using a template",
		Args:  cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			if flags.template == "" && flags.repo == "" {
				fmt.Fprintf(
					cmd.ErrOrStderr(),
					"Usage: codex create [dir] --template <template>\n\n",
				)
				templates.List(cmd.ErrOrStderr(), flags.showAll)
				if !flags.showAll {
					fmt.Fprintf(
						cmd.ErrOrStderr(),
						"\nTo see all available templates, run `codex create --show-all`\n",
					)
				}
				return nil
			}
	...
	command.Flags().StringVarP(
		&flags.repo, "repo", "r", "",
		"Git repository HTTPS URL to import template files from. Example: https://github.com/khulnasoft/codex",
	)
	command.Flags().StringVarP(
...
func runCreateCmd(cmd *cobra.Command, args []string, flags *createCmdFlags) error {
	...
	ux.Fsuccess(
		cmd.ErrOrStderr(),
		"Initialized codex project using template %s\n",
		flags.template,
	)
