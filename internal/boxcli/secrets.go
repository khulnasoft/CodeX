...
func secretsCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:             "Interact with codex secrets in khulnasoft cloud.",
...
func secretsInitCmd(secretsFlags *secretsFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "init",
		Short: "Initialize secrets management with khulnasoft cloud",
...
func secretsInitFunc(
	...
	// codex.Secrets() by default assumes project is initialized (and shows
	...
	if _, err := secrets.ProjectConfig(); err == nil &&
		...
		// Handle edge case where directory is already set up, but codex.json is
		// not configured to use khulnasoft-cloud.
		ux.Finfo(
			cmd.ErrOrStderr(),
			"Secrets already initialized. Adding to codex config.\n",
		)
	} else if err := secrets.NewProject(ctx, flags.force); err != nil {
		return errors.WithStack(err)
	}
	box.Config().Root.SetStringField("EnvFrom", "khulnasoft-cloud")
