...
func shellEnvCmd(defaults shellenvFlagDefaults) *cobra.Command {
	...
	command := &cobra.Command{
		Use:     "shellenv",
		Short:   "Print shell commands that create a Codex Environment in the shell",
	...
	command.Flags().BoolVar(
		&flags.pure, "pure", false, "if this flag is specified, codex creates an isolated environment inheriting almost no variables from the current environment. A few variables, in particular HOME, USER and DISPLAY, are retained.")
	command.Flags().BoolVar(
		...
		"preserves existing PATH order if this project's environment is already in PATH. "+
			"Useful if you want to avoid overshadowing another codex project that is already active")
	_ = command.Flags().MarkHidden("preserve-path-stack")
	command.Flags().BoolVar(
		&flags.noRefreshAlias, "no-refresh-alias", false,
		"by default, codex will add refresh alias to the environment"+
...
func shellEnvFunc(
	...
	if flags.recomputeEnv {
		ctx = ux.HideMessage(ctx, codex.StateOutOfDateMessage)
	}
