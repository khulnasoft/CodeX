...
import (
	...
	"github.com/khulnasoft/codex/internal/codex/docgen"
)
...
func dockerfileCmd() *cobra.Command {
	...
	command := &cobra.Command{
		Use:   "dockerfile",
		Short: "Generate a Dockerfile that replicates codex shell",
		Long: "Generate a Dockerfile that replicates codex shell. " +
			"Can be used to run codex shell environment in an OCI container.",
		Args: cobra.MaximumNArgs(0),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func direnvCmd() *cobra.Command {
	...
	command := &cobra.Command{
		Use:   "direnv",
		Short: "Generate a .envrc file that integrates direnv with this codex project",
		Long: "Generate a .envrc file that integrates direnv with this codex project. " +
	...
	command.Flags().BoolVarP(
		&flags.force, "force", "f", false, "force overwrite existing files")
	command.Flags().BoolVarP(
		&flags.printEnvrcContent, "print-envrc", "p", false, "output contents of codex configuration to use in .envrc")
...
func sshConfigCmd() *cobra.Command {
	...
	command := &cobra.Command{
		Use:    "ssh-config",
		Hidden: true,
		Short:  "Generate ssh config to connect to codex cloud",
		Long:   "Check ssh config and if they don't exist, it generates the configs necessary to connect to codex cloud VMs.",
		Args:   cobra.MaximumNArgs(0),
		RunE: func(cmd *cobra.Command, args []string) error {
...
func genAliasCmd() *cobra.Command {
	...
	command := &cobra.Command{
		...
		Long: "Generate shell script aliases for this project. " +
			"Usage is typically `eval \"$(codex gen alias)\"`.",
...
func runGenerateDirenvCmd(cmd *cobra.Command, flags *generateCmdFlags) error {
	if flags.printEnvrcContent {
		return codex.PrintEnvrcContent(
